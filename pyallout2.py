...
while True:
    ...
    if (len(faces) == 1 and len(mouth) == 0):
        cv2.putText(img, 'Wajah', (50, 50), font, 1, (0, 0, 0), 2, cv2.LINE_4)
         cv2.putText(img, 'Menggunakan Masker', (50, 80), font, 1, (0, 0, 0), 2, cv2.LINE_4)
        serialcomm.write("on".encode())
        ketmask = "yes"
    elif (len(faces) == 1 and len(mouth) == 1):
         cv2.putText(img, 'No Masker', (50, 80), font, 1, (0, 0, 0), 2, cv2.LINE_4)
        serialcomm.write("off".encode())
        ketmask = "no mask"
    elif (len(faces) == 0):
         cv2.putText(img, 'tdak ada wajah', (50, 80), font, 1, (0, 0, 0), 2, cv2.LINE_4)
        serialcomm.write("off".encode())
        ketmask = "nothing"
